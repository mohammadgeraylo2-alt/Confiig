    ...
    for i in range(6):
        msg = await act_and_wait_any(
            ...
            lambda e: (
                ...
                or _has_button_containing(e, "✅")
                or (
                    e.buttons and len(e.buttons) == 3
                    and len(e.buttons[0]) == 3 and len(e.buttons[1]) == 1 and len(e.buttons[2]) == 1
                )
            ),
        ...
        pishi_count_confirm_btn = (
            _find_button_containing(msg, "✅")
            or (
                msg.buttons and len(msg.buttons) == 3
                and len(msg.buttons[0]) == 3 and len(msg.buttons[1]) == 1 and len(msg.buttons[2]) == 1
                and _get_button_at(msg, 1, 0)
            )
        )
        if pishi_count_confirm_btn:
            # صفحه انتخاب تعداد پیشی برای قاچاق (➖ ➕ ➕ / ✅ / BK) - دکمه ✅ این
            # یکی معمولا ایموجی سفارشی/پرمیومه و متنش خالیه، پس اگه با متن پیدا
            # نشد بر اساس موقعیت (ردیف ۱، ستون ۰) کلیک می‌کنیم تا تعداد پیش‌فرض
            # تایید بشه و به صفحه شروع قاچاق برسیم
            await pishi_count_confirm_btn.click()
