...
async def run_smuggling_cycle(label, client):
    ...
    for _ in range(6):
        ...
        logger.info(f"[{label}] پیام 'قاچاق میویی' ارسال شد")
        ...
        if _has_button_containing(msg, "زندان") or _has_button_containing(msg, "آزاد"):
            ...
            jail_msg = await wait_for_event(
                client, group, lambda e: _has_button_containing(e, "جریمه"), timeout=20,
            )
            await _find_button_containing(jail_msg, "جریمه").click()

            confirm_jail = await wait_for_event(
                client, group, lambda e: _has_button_containing(e, "✅"), timeout=15,
            )
