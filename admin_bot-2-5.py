...
async def fishing_loop(label, client):
    while True:
        ...
        try:
            ...
            logger.warning(f"[{label}] خطا توی چرخه ماهیگیری: {e}")
...
async def run_fishing_cycle(label, client):
    ...
    # ۱. درخواست ماهی
    await client.send_message(group, "ماهی")
    logger.info(f"[{label}] پیام 'ماهی' ارسال شد")

    caught = await wait_for_event(
        client, group,
        lambda e: e.raw_text and "با موفقیت" in e.raw_text and "گرفتید" in e.raw_text,
        timeout=30,
    )
    ...
    if btn:
        ...
        for _ in range(3):
            try:
                btn_msg = await wait_for_event(
                    client, group, lambda e: _has_button_containing(e, "یخچال"), timeout=8,
                )
    ...
    await client.send_message(group, "یخچال میویی")
    fridge_msg = await wait_for_event(
        client, group,
    ...
    if not fish_btn:
        ...
        return
    await fish_btn.click()

    action_msg = await wait_for_event(
        client, group, lambda e: _has_button_containing(e, "پخیدن"), timeout=15,
    )
    await _find_button_containing(action_msg, "پخیدن").click()

    confirm_msg = await wait_for_event(
        client, group, lambda e: _has_button_containing(e, "✅"), timeout=15,
    )
    ...
    await client.send_message(group, "یخچال میویی")
    fridge_msg2 = await wait_for_event(
        client, group,
        lambda e: _has_button_containing(e, "🐟") or _has_button_containing(e, "🐠"),
        timeout=20,
    )
    ...
    if not fish_btn2:
        ...
        return
    await fish_btn2.click()

    sell_msg = await wait_for_event(
        client, group, lambda e: _has_button_containing(e, "فروش"), timeout=15,
    )
...
async def smuggling_loop(label, client):
    while True:
        try:
            ...
            logger.warning(f"[{label}] خطا توی چرخه قاچاق: {e}")
...
async def run_smuggling_cycle(label, client):
    ...
    # ۱. شروع دور جدید قاچاق
    await client.send_message(group, "قاچاق میویی")
    logger.info(f"[{label}] پیام 'قاچاق میویی' ارسال شد (شروع)")

    start_msg = await wait_for_event(
        client, group,
        lambda e: _has_button_containing(e, "شروع") and _has_button_containing(e, "قاچاق"),
        timeout=20,
    )
    await _find_button_containing(start_msg, "شروع").click()
    ...
    # ۴. دوباره بفرست تا نتیجه و دکمه دریافت دستمزد بیاد
    await client.send_message(group, "قاچاق میویی")
    result_msg = await wait_for_event(
        client, group, lambda e: _has_button_containing(e, "دستمزد"), timeout=30,
    )
