...
async def run_casino_cycle(label, client):
    ...
    # این دکمه‌ها هم مثل منوی اصلی از ایموجی سفارشی استفاده می‌کنن (متن‌شون خالیه)،
    # پس با موقعیت شناسایی می‌کنیم: یه ردیف تکی با دقیقا ۳ دکمه [۱, ۲, ۳]
    numbers_msg = await act_and_wait_any(
        client, group, players_btn.click,
        lambda e: e.buttons and len(e.buttons) == 1 and len(e.buttons[0]) == 3,
