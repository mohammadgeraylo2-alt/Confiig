...
--------------------------------------------
این نسخه مستقیم از telethon با API_ID/API_HASH واقعی خودت استفاده می‌کنه
(باید از my.telegram.org بگیریش). دیگه از api فیک opentele2 استفاده نمی‌کنه
که باعث خراب شدن ارسال کد و لاگین می‌شد.
...
PHONE, CODE, PASSWORD, GROUP, MESSAGE, INTERVAL = range(6)
...
# ---------------------------------------------------------------------------
pending_approvals = {}   # token -> {"client","label","requester_id","requester_name"}
...
async def addaccount_phone(update, context):
    ...
    await update.message.reply_text(
        ...
        "⚠️ بین رقم‌ها یه فاصله یا حرف اضافه بذار (مثلا 3 3 1 3 9)، وگرنه چون همون کد رو داری تو یه چت دیگه‌ی "
        "تلگرام تایپ می‌کنی، سیستم امنیتی تلگرام فکر می‌کنه داری کدتو به اشتراک می‌ذاری و باطلش می‌کنه."
    )
    return CODE
...
async def ask_group(update, context):
    await update.message.reply_text(
        "لینک/یوزرنیم/آیدی گپی که این اکانت باید توش پیام بده رو بفرست\n"
        "(یا بزن - تا از گروه پیش‌فرض ربات استفاده کنه):"
    )
    return GROUP


async def addaccount_group(update, context):
    text = update.message.text.strip()
    info = pending[update.effective_user.id]
    info["group"] = None if text == "-" else text
    await update.message.reply_text(
        "متن پیامی که این اکانت باید بفرسته رو بنویس\n"
        "(یا بزن - تا از پیام پیش‌فرض ربات استفاده کنه):"
    )
    return MESSAGE


async def addaccount_message(update, context):
    text = update.message.text.strip()
    info = pending[update.effective_user.id]
    info["message"] = None if text == "-" else text
    await update.message.reply_text(
        "فاصله زمانی ارسال این اکانت رو بفرست، مثلا: 300 360 (بر حسب ثانیه)\n"
        "(یا بزن - تا از فاصله پیش‌فرض ربات استفاده کنه):"
    )
    return INTERVAL


async def addaccount_interval(update, context):
    text = update.message.text.strip()
    info = pending[update.effective_user.id]
    if text == "-":
        info["min_delay"] = info["max_delay"] = None
    else:
        try:
            mn, mx = map(int, text.split())
            if mn <= 0 or mx <= 0:
                raise ValueError
            if mn > mx:
                mn, mx = mx, mn
        except ValueError:
            await update.message.reply_text("فرمت درست نیست. مثلا بفرست: 300 360")
            return INTERVAL
        info["min_delay"], info["max_delay"] = mn, mx

    return await finish_login(update, context)


async def complete_login(client, label, me, group=None, message=None, min_delay=None, max_delay=None, owner=None):
    """سشن رو ذخیره می‌کنه و اگه گروهی (اختصاصی یا پیش‌فرض) تنظیم شده باشه، اکانت رو استارت می‌کنه."""
    data["accounts"][label] = {
        "session": client.session.save(), "active": True,
        "group": group, "message": message,
        "min_delay": min_delay, "max_delay": max_delay,
        "owner": owner,
    }
    save_data()
    if group or settings["group"]:
        start_worker(label, client)
    else:
        await client.disconnect()


def owns_account(user_id, label):
    """ادمین همیشه دسترسی کامل داره؛ بقیه فقط به اکانتی که خودشون اضافه کردن."""
    if user_id == ADMIN_ID:
        return True
    return data["accounts"].get(label, {}).get("owner") == user_id
...
async def finish_login(update, context):
    ...
    group, message = info.get("group"), info.get("message")
    min_delay, max_delay = info.get("min_delay"), info.get("max_delay")

    # خود ادمین: بدون تاییدیه مستقیم اضافه میشه
    if requester_id == ADMIN_ID:
        await complete_login(client, label, me, group, message, min_delay, max_delay, owner=ADMIN_ID)
        await update.message.reply_text(f"اکانت {me.first_name} ({label}) با موفقیت اضافه شد.")
        if not group and not settings["group"]:
            await update.message.reply_text(
                "نه گروه اختصاصی این اکانت و نه گروه پیش‌فرض تنظیم نشده. با /setgroup یکیشو تنظیم کن."
            )
        return ConversationHandler.END

    # هرکس دیگه: باید ادمین تایید کنه
    token = uuid.uuid4().hex[:8]
    pending_approvals[token] = {
        "client": client, "label": label,
        "requester_id": requester_id, "requester_name": update.effective_user.full_name,
        "group": group, "message": message,
        "min_delay": min_delay, "max_delay": max_delay,
    }
    username = f"@{me.username}" if me.username else "(بدون یوزرنیم)"
    keyboard = InlineKeyboardMarkup([[
        InlineKeyboardButton("✅ قبول", callback_data=f"approve:{token}"),
        InlineKeyboardButton("❌ رد", callback_data=f"reject:{token}"),
    ]])
    await context.bot.send_message(
        ...
        f"نام: {me.first_name}\n"
        f"یوزرنیم: {username}\n"
        f"درخواست‌دهنده: {update.effective_user.full_name} (آیدی: {requester_id})\n"
        f"گپ: {group or '(پیش‌فرض)'}\n"
        f"پیام: {message or '(پیش‌فرض)'}\n"
        f"فاصله ارسال: {f'{min_delay} تا {max_delay} ثانیه' if min_delay else '(پیش‌فرض)'}",
        reply_markup=keyboard,
    )
    await update.message.reply_text("درخواستت برای ادمین فرستاده شد، منتظر تایید بمون.")
    return ConversationHandler.END


async def handle_login_approval(update, context):
    query = update.callback_query
    if query.from_user.id != ADMIN_ID:
        await query.answer("فقط ادمین می‌تونه تایید/رد کنه.", show_alert=True)
        return
    await query.answer()

    action, token = query.data.split(":", 1)
    info = pending_approvals.pop(token, None)
    if not info:
        await query.edit_message_text("این درخواست دیگه معتبر نیست (شاید قبلا رسیدگی شده).")
        return
    ...
    if action == "approve":
        ...
        await complete_login(
            client, label, me,
            info.get("group"), info.get("message"),
            info.get("min_delay"), info.get("max_delay"),
            owner=requester_id,
        )
        await query.edit_message_text(query.message.text + "\n\n✅ تایید شد و اضافه شد.")
        await context.bot.send_message(requester_id, f"اکانت {label} تایید و اضافه شد ✅")
    else:
        await client.disconnect()
        await query.edit_message_text(query.message.text + "\n\n❌ رد شد.")
        await context.bot.send_message(requester_id, f"درخواست لاگین اکانت {label} توسط ادمین رد شد ❌")
...
async def worker_loop(label, client):
    ...
    while True:
        ...
        try:
            group = _resolve_group(acc.get("group") or settings["group"])
            message = acc.get("message") or settings["message"]
        ...
        try:
            mn = acc.get("min_delay") or settings["min_delay"]
            mx = acc.get("max_delay") or settings["max_delay"]
            if mn > mx:
...
async def run_fishing_cycle(label, client):
    ...
    action_msg = await act_and_wait_any(
        ...
        lambda e: _has_button_containing(e, "بپوخش") or _has_button_containing(e, "پخیدن"),
        "پیام منوی ماهی با دکمه بپوخش/پخیدن", label, timeout=15,
    )
    pokh_btn = (
        _find_button_containing(action_msg, "بپوخش")
        or _find_button_containing(action_msg, "پخیدن")
    )
    if not pokh_btn:
        logger.warning(f"[{label}] دکمه بپوخش/پخیدن پیدا نشد، این دور رو رد می‌کنیم")
        return
...
async def run_streak_casino_cycle(label, client):
    ...
    # ۷-۸. کلیک روی ساخت میز قمار، هم‌زمان با ثبتِ منتظرِ پنل انتخاب فرد/زوج.
    # نکته‌ی مهم: باید handler رو *قبل* از کلیک ثبت کنیم، نه بعدش - وگرنه اگه
    # جواب بازی سریع‌تر از ثبت‌شدن handler برسه (که طبق لاگ واقعاً این‌طور
    # می‌شد)، پیام از دست می‌ره و کد بی‌خودی تایم‌اوت می‌خوره با اینکه پنل
    # درست همون لحظه اومده بوده. برای همین این‌جا از act_and_wait_any با خودِ
    # build_btn.click به‌عنوان action استفاده می‌کنیم (نه بعد از کلیکِ جدا).
    # شکل پنل: ردیف ۰ = ۲ دکمه (فرد/زوج)، ردیف ۱ و ۲ = هر کدوم ۳ دکمه (۱-۶).
    # بازی ۶۰ ثانیه فرصت انتخاب می‌ده، پس timeout رو هم‌قدم با اون می‌ذاریم.
    try:
        parity_panel = await act_and_wait_any(
            client, group, build_btn.click,
            lambda e: (
                e.buttons and len(e.buttons) == 3
                and len(e.buttons[0]) == 2 and len(e.buttons[1]) == 3 and len(e.buttons[2]) == 3
            ),
            "پنل انتخاب فرد/زوج بعد از ساخت میز", label, timeout=55,
        )
    except asyncio.TimeoutError:
        return
...
async def cmd_list(update, context):
    ...
    labels = list(data["accounts"]) if user_id == ADMIN_ID else [
        l for l in data["accounts"] if data["accounts"][l].get("owner") == user_id
    ]
    if not labels:
        await update.message.reply_text("هیچ اکانتی نداری." if user_id != ADMIN_ID else "هیچ اکانتی اضافه نشده.")
        return
    lines = []
    for label in labels:
        ...
        if label == settings.get("streak_casino_account"):
            tag += " 🎲 کازینوی رشته‌ای"
        acc = data["accounts"][label]
        gm = acc.get("group") or "پیش‌فرض"
...
async def cmd_stop(update, context):
    if not context.args:
        ...
        return
    label = context.args[0]
    if not owns_account(update.effective_user.id, label):
        await update.message.reply_text("این اکانت مال تو نیست.")
        return
...
async def cmd_import(update, context):
    ...
    data["accounts"][label] = {"session": session_string, "active": True, "owner": ADMIN_ID}
    save_data()
...
async def cmd_start(update, context):
    if update.effective_user.id != ADMIN_ID:
        await update.message.reply_text(
            "👋 دستورات تو\n\n"
            "/addaccount — اضافه کردن اکانت با شماره (بعد از تایید ادمین فعال میشه)\n"
            "/list — لیست اکانت‌های خودت\n"
            "/startaccount شماره — استارت اکانت خودت\n"
            "/stopaccount شماره — استاپ اکانت خودت"
        )
        return
    await update.message.reply_text(
        "👋 دستورات ربات\n\n"
        "📱 اکانت‌ها\n"
        "/addaccount — اضافه کردن اکانت با شماره (اگه خودت بزنی مستقیم، اگه بقیه بزنن نیاز به تایید تو داره)\n"
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            GROUP: [MessageHandler(filters.TEXT & ~filters.COMMAND, addaccount_group)],
            MESSAGE: [MessageHandler(filters.TEXT & ~filters.COMMAND, addaccount_message)],
            INTERVAL: [MessageHandler(filters.TEXT & ~filters.COMMAND, addaccount_interval)],
        },
        fallbacks=[CommandHandler("cancel", cancel)],
    )

    app.add_handler(conv)
    app.add_handler(CallbackQueryHandler(handle_login_approval, pattern=r"^(approve|reject):"))
