...
settings = {
    ...
    "rescue_account": None,         # کدوم اکانت دکمه "نجات دادن میو" رو کلیک کنه
}
...
async def _try_click_rescue_button(event, label):
    ...
    for row in event.buttons:
        for button in row:
            if _button_matches_rescue(button):
                try:
                    await button.click()
                    logger.info(f"[{label}] دکمه '{button.text}' کلیک شد (نجات میو)")
                except Exception as e:
                    logger.warning(f"[{label}] خطا در کلیک دکمه نجات میو: {e}")
                return
...
def attach_rescue_handler(label, client):
    ...
    async def _on_new(event, label=label):
        await _try_click_rescue_button(event, label)
