    ...
    for i in range(6):
        msg = await act_and_wait_any(
            ...
            lambda e: (
                ...
                or (e.raw_text and "قاچاق پیشی" in _clean_text(e.raw_text))
            ),
            f"پاسخ به قاچاق میویی (شروع/دستمزد/زندان/انتخاب تعداد پیشی) (تلاش {i+1}/6)", label, timeout=25,
        )

        if msg.raw_text and "قاچاق پیشی" in _clean_text(msg.raw_text):
            # صفحه انتخاب تعداد پیشی برای قاچاق - همون تعداد پیش‌فرض رو با ✅ تایید می‌کنیم
            confirm_count_btn = _find_button_containing(msg, "✅")
            if confirm_count_btn:
                await confirm_count_btn.click()
                logger.info(f"[{label}] پنل انتخاب تعداد پیشی قاچاقی: تعداد پیش‌فرض با ✅ تایید شد")
            else:
                logger.warning(f"[{label}] پنل انتخاب تعداد پیشی قاچاقی: دکمه ✅ پیدا نشد")
            await asyncio.sleep(2)
            continue
