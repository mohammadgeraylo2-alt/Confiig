...
settings = {
    ...
    "casino_account": None,   # فقط همین اکانت هر ۵ دقیقه "کازینو" رو می‌فرسته
}
...
# ---------------------------------------------------------------------------
# روتین مستقل کازینو (هر ۵ دقیقه پیام "کازینو" ارسال می‌شه)
# ---------------------------------------------------------------------------
CASINO_INTERVAL = 5 * 60  # هر ۵ دقیقه
...
async def run_casino_cycle(label, client):
    ...
    # ۱. ارسال «کازینو» و صبر برای منوی بازی‌ها (دکمه 🎰 گردونه شانس)
    menu_msg = await act_and_wait_any(
        client, group,
        lambda: client.send_message(group, "کازینو"),
        lambda e: _has_button_containing(e, "🎰"),
        "منوی کازینو (دکمه 🎰 گردونه شانس)", label, timeout=30,
    )
    slot_btn = _find_button_containing(menu_msg, "🎰")
    if not slot_btn:
        logger.warning(f"[{label}] دکمه 🎰 توی منوی کازینو پیدا نشد")
        return
    ...
    numbers_msg = await act_and_wait_any(
        ...
        lambda e: _find_number_button(e, 1) is not None,
        "پیام انتخاب تعداد بازیکن (دکمه‌های 1/2/3)", label, timeout=15,
    )
    one_btn = _find_number_button(numbers_msg, 1)
    if not one_btn:
        logger.warning(f"[{label}] دکمه '1' برای تعداد بازیکن پیدا نشد")
        return
    ...
    logger.info(f"[{label}] ایموجی 🎰 (قبل از ساخت میز) ارسال شد، ۵ ثانیه صبر می‌کنیم")
    ...
    # ۹. دوباره ریپلای با 🎰 روی همون پنل نهایی - پایان چرخه
    await client.send_message(group, "🎰", reply_to=final_panel.id)
    logger.info(f"[{label}] ایموجی 🎰 (بعد از ساخت میز) ارسال شد ✅ چرخه کازینو تموم شد")
...
async def cmd_start(update, context):
    await update.message.reply_text(
        ...
        "/setcasino شماره - انتخاب اکانتی که هر ۵ دقیقه یک دور کامل گردونه شانس کازینو رو انجام بده (off برای غیرفعال کردن)\n"
        "/list - لیست اکانت‌ها\n"
