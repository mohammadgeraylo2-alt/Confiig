...
STREAK_BET_THROW_DELAY = (1, 2)  # فاصله بین تاس‌های سیو مسیج (ثانیه، رندوم)
...
async def run_streak_dice_phase(label, client, target_streak=STREAK_BET_TARGET):
    """تو Saved Messages تاس واقعی می‌ندازه (و پیشرفت رو بعد از هر پرتاب سیو
    می‌کنه تا اگه ربات وسط کار ری‌استارت شد از همون‌جا ادامه بده) تا وقتی
    target_streak تا پشت‌سرهم فرد یا زوج بگیره. پاریتی مخالفِ رشته رو برمی‌گردونه
    (مثلا اگه ۸ تا فرد اومد، 'زوج' رو برمی‌گردونه تا رو گروه روش شرط ببندیم)."""
    ...
    while True:
        ...
        await asyncio.sleep(random.uniform(*STREAK_BET_THROW_DELAY))
...
async def run_streak_casino_cycle(label, client):
    ...
    # ۷. ریپلای واقعی (InputMediaDice) با 🎰 روی پنل نهایی، بعد ۵ ثانیه صبر
    await client.send_file(group, InputMediaDice(emoticon="🎰"), reply_to=final_panel.id)
    logger.info(f"[{label}] (کازینوی رشته‌ای) ایموجی 🎰 ۱/۲ ارسال شد، ۵ ثانیه صبر می‌کنیم")
    await asyncio.sleep(5)
    ...
    # ۹. بعد از ساخت میز، پنل انتخاب فرد/زوج (+ اعداد ۱ تا ۶) میاد - دقیقا
    ...
    # ۱۰. دوباره ریپلای واقعی (InputMediaDice) با 🎰 روی همون پنل نهایی - پایان چرخه
    await client.send_file(group, InputMediaDice(emoticon="🎰"), reply_to=final_panel.id)
    logger.info(f"[{label}] (کازینوی رشته‌ای) ایموجی 🎰 ۲/۲ ارسال شد ✅ چرخه تموم شد")

    # ۱۱. صبر برای پیام نتیجه (برد/باخت) و به‌روزرسانی آمار سود/ضرر
    try:
        result_msg = await act_and_wait_any(
            client, group, lambda: asyncio.sleep(0),
            lambda e: (
...
async def cmd_start(update, context):
    await update.message.reply_text(
        ...
        "/setpishi شماره - انتخاب اکانتی که هر ۶ ساعت «پیشی» رو می‌فرسته و میو پوینت‌ها رو برداشت می‌کنه (off برای غیرفعال کردن)\n"
        "/dicestreak شماره [تعداد] - تو Saved Messages همون اکانت تاس می‌ندازه تا N تا پشت‌سرهم زوج یا فرد بگیره (پیش‌فرض ۸)\n"
        "/stopdice شماره - متوقف کردن روتین تاس یک اکانت\n"
        "/setstreakcasino شماره - انتخاب اکانتی که تو Saved Messages تا ۸ تاس پشت‌سرهم فرد/زوج می‌ندازه، بعد تو گروه رو مخالفش شرط می‌بنده (off برای غیرفعال کردن)\n"
        "/streakcasinostats - نمایش آمار سود/ضرر کازینوی رشته‌ای (reset برای صفر کردن)\n"
        "/list - لیست اکانت‌ها\n"
